...
BOUNCERATE = 8       # how fast the player bounces (large is slower)
...
def main():
    ...
    L_SQUIR_IMG = pygame.image.load('knight_transparent.png')
    R_SQUIR_IMG = pygame.transform.flip(L_SQUIR_IMG, True, False)
    global BGINFO
    BGINFO = pygame.image.load('cracked_earth.png').convert()
    BGINFO = pygame.transform.scale(BGINFO, (WINWIDTH, WINHEIGHT))

    pygame.mixer.init()
    pygame.mixer.music.load('decision_time.mp3')
    pygame.mixer.music.set_volume(0.5)
    pygame.mixer.music.play(-1)

    while True:
        # Load the main theme at the start o every play
        pygame.mixer.music.load('decision_time.mp3')
        pygame.mixer.music.play(-1)
...
def runGame():
    ...
    winSurf = BASICFONT.render('You have achieved MEGA KNIGHT!', True, WHITE)
    ...
    while True: # main game loop
        ...
        offsetX = -camerax % WINWIDTH
        offsetY = -cameray % WINHEIGHT

        for x in range(-1, 2):
            for y in range(-1, 2):
                DISPLAYSURF.blit(BGINFO, (offsetX + (x * WINWIDTH), offsetY + (y * WINHEIGHT)))
        ...
        for event in pygame.event.get(): # event handling loop
            if event.type == QUIT:
                ...
                if event.key in (K_UP, K_w):
                    ...
                    return
                # Add this to remove the end screen timer
